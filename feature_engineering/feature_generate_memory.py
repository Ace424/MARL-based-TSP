...
def binning_with_tree(ori_fe: np.array,
    ...
    if label is not None:
        if col_index not in memory:
            boundry = []
            clf = DecisionTreeClassifier(criterion='entropy',  # “信息熵”最小化准则划分
                                         max_leaf_nodes=6,  # 最大叶子节点数
                                         min_samples_leaf=0.05)  # 叶子节点样本数量最小占比
            fe = ori_fe.reshape(-1, 1)
            clf.fit(fe, label)  # 训练决策树
            ...
            if memory is not None:
                memory[col_index] = boundry
        else:
            boundry = memory[col_index]
    else:
        assert col_index in memory, 'Col index not in boundry memory,check your code!'
        boundry = memory[col_index]
    if len(boundry):
